...
conn = sqlite3.connect('student.db') # 접속객체
mycursor = conn.cursor() # 커서 객체
...
mycursor.execute("""
insert into student
    values('lee','이민혁','1989/12/12','M','컴퓨터공학과',4,'010-1111-2222')
""")
...
student_list = [
    ...
    ('sim','심유이','195/07/17','F','연극영화과',2,'010-2222-6666')
]
...
print('\n# like 검색')
findword = '이'
sql = f"select * from student where name like '%{findword}%'"
print(f'이름에 "{findword}"라는 글자가 포함된 사람')
mycursor.execute(sql)
for row in mycursor.fetchall():
    print(row)

#execute() 메소드의 2번째 매개 변수는 tuple 또는 list
print('\n# update 구문')
pid, newname = 'lee','이순철'
