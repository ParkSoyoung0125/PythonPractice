...
regEx = '\d+' # 숫자가 1개 이상이어야함
pattern = re.compile(regEx)

result = pattern.match(mystring)
print('match 함수는 문자열의 처음부터 체크함.')
print('match 결과 : %s' % result)

result = pattern.search(mystring)
print('search 함수는 임의의 위치에서 조건에 맞으면 출력해줌.')
...
print('match 함수와 search 함수의 실행결과는 동일함.')

print('\nmatch 함수의 반환결과는 Match 객체임.')
print('Match 객체는 색인의 위치나 문자열 정보를 위한 여러 개의 메소드가 존재함.')
mystring = 'python1234'
...
print('방법01')
regEx = '\d\S*' # 숫자로 시작한 이후에 white character 문자가 아닌 것들
pattern = re.compile(regEx)
