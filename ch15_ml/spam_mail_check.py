...
from ch15_ml.svm_titanic import x_train, x_test, y_train, y_test
...
print('\n메일 제목 token으로 만들기')
num_to_show = 5 # 보여줄 데이터 갯수

print('변수 emails는 메일의 제목과 스팸 여부를 tuple 형식으로 갖고있는 list')
emails = [tuple(row) for row in email_frame.itertuples(index=False)]
print(emails[0:num_to_show])

print('\n변수 email_tokenized는 메일의 제목이 토큰화 된 데이터임.')
print('변수 subject는 이메일 제목, label는 스팸 여부 문자열임.')
email_tokenized = [(tokenize(subject),label) for subject,label in emails]
print(email_tokenized[0:num_to_show])

print('\n독립 변수와 종속 변수로 분리함.')
x, y = zip(*email_tokenized)
...
#   Coords   Values
#   (0, 107)   1 <-- 0번째 문서에서 107에 해당하는 단어가 1번 나왔슴
#   (0, 43)   1 <-- 0번째 문서에서 43에 해당하는 단어가 1번 나왔음
...
su_list = [107, 43, 1]
for su in su_list:
    print(f'{su}번에 해당하는 단어 : {inv_vocab}')
#end for
...
print('\n실제 값(actual)')
print(y_test) # 실제 정답을 의미하며, 머신 러닝에서 label이라고 부름
...
print('\n각 메일을 반복하여 스팸 여부를 체크함.')
final_email_info = [] # 최종 결과를 저장할 리스츠
